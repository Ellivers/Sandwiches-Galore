...
execute unless data storage sandwiches:galore Items[{Slot:13b,id:"minecraft:white_stained_glass_pane",tag:{SG:{GUI_Item:1b}}}] run function sandwiches_galore:machines/slots/13_oven_on
...
execute if score @s sg.process matches 1..22 store result block ~ ~ ~ Items[{Slot:13b,tag:{SG:{GUI_Item:1b}}}].tag.CustomModelData int 1 run scoreboard players get #progress_temp sandwiches