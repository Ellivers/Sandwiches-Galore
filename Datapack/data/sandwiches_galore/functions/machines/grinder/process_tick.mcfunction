...
execute unless data storage sandwiches:galore Items[{Slot:13b,id:"minecraft:gray_stained_glass_pane",tag:{SG:{GUI_Item:1b}}}] run function sandwiches_galore:machines/slots/13_grinder_on
# Update the progress item's visuals
scoreboard players operation #progress_temp sandwiches = @s sg.process
scoreboard players add #progress_temp sandwiches 6710000
execute if score @s sg.process matches 1..16 store result block ~ ~ ~ Items[{Slot:13b,tag:{SG:{GUI_Item:1b}}}].tag.CustomModelData int 1 run scoreboard players get #progress_temp sandwiches
