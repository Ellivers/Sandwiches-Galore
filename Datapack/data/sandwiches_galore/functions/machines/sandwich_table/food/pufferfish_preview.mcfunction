...
execute unless score #sg.name_level sandwiches matches 11.. run data modify block ~ ~ ~ Items[{Slot:16b}].tag.display.Name set value '{"italic":false,"translate":"item.sandwiches_galore.sandwich.pufferfish"}'
data modify block ~ ~ ~ Items[{Slot:16b}].tag.display.Lore prepend value '[{"text":"- ","color":"gray","italic":false},{"translate":"item.minecraft.pufferfish"}]'
execute store result score #sg.custom_model_data sandwiches run data get storage sandwiches:sandwich_table Items[{Slot:10b}].tag.CustomModelData
scoreboard players add #sg.custom_model_data sandwiches 5
execute store result block ~ ~ ~ Items[{Slot:16b}].tag.CustomModelData int 1 run scoreboard players get #sg.custom_model_data sandwiches