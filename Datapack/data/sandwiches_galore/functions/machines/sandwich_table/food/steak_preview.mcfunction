...
execute unless score #sg.name_level sandwiches matches 10.. run data modify block ~ ~ ~ Items[{Slot:16b}].tag.display.Name set value '{"italic":false,"translate":"item.sandwiches_galore.sandwich.cooked_beef"}'
data modify block ~ ~ ~ Items[{Slot:16b}].tag.display.Lore prepend value '[{"text":"- ","color":"gray","italic":false},{"translate":"item.minecraft.cooked_beef"}]'