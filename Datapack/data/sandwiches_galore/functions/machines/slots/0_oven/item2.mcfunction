...
scoreboard players operation #tempcount sg.dummy = #slot_2 sg.dummy
scoreboard players operation #tempcount sg.dummy += #slot_0 sg.dummy

# Only merge until it reaches 64 items
execute if score #tempcount sg.dummy matches 65.. run data modify block ~ ~ ~ Items[{Slot:2b}].Count set value 64b
...
execute unless score #tempcount sg.dummy matches 65.. store result block ~ ~ ~ Items[{Slot:2b}].Count byte 1 run scoreboard players get #tempcount sg.dummy
